...
from dataloaders.dataloader_frog import FrogDataset
from models.stfts import mag_phase_stft, mag_phase_istft
from models.generator import FrogPriorMamba
...
def create_freq_weight_matrix(n_fft, sr, device):
    """
    根据 33 个蛙鸣频带分析结果（freq_band_detection.py），
    构造频域损失权重矩阵。
    STFT 输出的频率维度 F = n_fft // 2 + 1
    """
    F = n_fft // 2 + 1
    ...
    # A: 低频噪声压制 (0 ~ 1000 Hz, 平均能量 ~12%, 权重稍降防止过度拟合)
    noise_mask = freqs <= 1000.0
    weights[noise_mask] = 3.0

    # B: 蛙鸣主频带保护 (1529 ~ 2500 Hz, 平均能量 ~25%, 最核心区域)
    #    上限从 2317 拓宽至 2500，覆盖 2400 Hz 附近的能量峰值
    main_mask = (freqs >= 1529.2) & (freqs <= 2500.0)
    weights[main_mask] = 3.5

    # C: 核心峰值区 (1668 ~ 2050 Hz, B 的子集，最强能量聚集区)
    #    上限从 1973 拓宽至 2050，覆盖 2000~2040 Hz 的多个蛙鸣最强峰
    peak_mask = (freqs >= 1668.3) & (freqs <= 2050.0)
    weights[peak_mask] = 4.5

    # E: 中频过渡带保护 (2500 ~ 4000 Hz, 新增)
    #    此区域有部分不可忽略的峰值 (2680~2920 Hz 等)，防误伤
    mid_mask = (freqs >= 2500.0) & (freqs <= 4000.0)
    weights[mid_mask] = 1.5

    # D: 高频谐波保护 (4000 ~ 4800 Hz, 平均能量 ~2.5%)
    #    范围从 4050~4346 拓宽至 4000~4800，覆盖更完整的谐波区域
    sec_mask = (freqs >= 4000.0) & (freqs <= 4800.0)
    weights[sec_mask] = 1.5

    weights = weights / weights.mean()
    # 调整形状以便在 Loss 计算时可以进行广播 
    # [B, 1, F, T] 对应的后三个维度是 1, F, 1
    weights_mag = weights.view(1, 1, F, 1)
    
    # 对于复数谱，形状通常多一个实虚部维度：[B, 1, F, T, 2]
    weights_com = weights.view(1, 1, F, 1, 1)
    
    return weights_mag, weights_com
...
def train(rank, args, cfg):
    ...
    generator = FrogPriorMamba(cfg).to(device)
    ...
    if state_dict_g is not None:
        # strict=False 允许灵活加载（如只加载部分权重 fine-tune）
        missing_g, unexpected_g = generator.load_state_dict(state_dict_g['generator'], strict=False)
        if missing_g:
            print(f"[Warning] Generator missing keys: {missing_g}")
        if unexpected_g:
            print(f"[Warning] Generator unexpected keys: {unexpected_g}")
        missing_d, unexpected_d = discriminator.load_state_dict(state_dict_do['discriminator'], strict=False)
        if missing_d:
            print(f"[Warning] Discriminator missing keys: {missing_d}")
        if unexpected_d:
            print(f"[Warning] Discriminator unexpected keys: {unexpected_d}")
    ...
    freq_weights_mag, freq_weights_com = create_freq_weight_matrix(n_fft, sr, device)
    # ========================================================
    ...
    for epoch in range(max(0, last_epoch), cfg['training_cfg']['training_epochs']):
        ...
        for i, batch in enumerate(train_loader):
            ...
            audio_list_r, audio_list_g = list(clean_audio.detach().cpu().numpy()), list(audio_g.detach().cpu().numpy())
            batch_sisdr_score = batch_sisdr(audio_list_r, audio_list_g, cfg)

            # Discriminator
            optim_d.zero_grad()
            # 真实分支: (noisy, clean) → 理想的增强映射, 预测 1
            metric_r = discriminator(noisy_mag, clean_mag)
            loss_disc_r = F.mse_loss(one_labels, metric_r.flatten())
            # 虚假分支: (noisy, enhanced) → 预测 SI-SDR (离完美还有多远)
            metric_g = discriminator(noisy_mag, mag_g.detach())
            ...
            # ======== 加权的幅度谱与复数谱损失 ========
            # L2 Magnitude Loss (Weighted)
            loss_mag = torch.mean(freq_weights_mag * (clean_mag - mag_g) ** 2)
            # Anti-wrapping Phase Loss
            loss_ip, loss_gd, loss_iaf = phase_losses(clean_pha, pha_g, cfg)
            loss_pha = loss_ip + loss_gd + loss_iaf
            # L2 Complex Loss (Weighted)
            loss_com = torch.mean(freq_weights_com * (clean_com - com_g) ** 2) * 2
            # ==========================================

            # Time Loss
            loss_time = F.l1_loss(clean_audio, audio_g)
            # 对抗 Loss: 生成器想让判别器认为 enhanced = ideal
            metric_g = discriminator(noisy_mag, mag_g)
            ...
            if rank == 0:
                ...
                if steps % cfg['env_setting']['validation_interval'] == 0 and steps != 0:
                    ...
                    # Select the best checkpoint by validation SI-SDR.
...
def main():
    ...
    parser.add_argument('--exp_name', default='FrogPriorMamba')
    parser.add_argument('--config', default='recipes/FrogPriorMamba/FrogPriorMamba.yaml')
