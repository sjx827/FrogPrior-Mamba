...
class FrogDataset(torch.utils.data.Dataset):
    """
    Dataset for loading clean and noisy audio files.

    Args:
        clean_json (str): JSON file containing clean audio paths.
        noisy_json (str): JSON file containing noisy audio paths.
    ...
    def __init__(
        ...
        # DataLoader or DistributedSampler controls shuffling.
        ...
        # Cache by index when n_cache_reuse is enabled.
        self._cache = {}
        self.device = device

    def __getitem__(self, index):
        # Reuse cached audio for this index when available.
        cached = self._cache.get(index)
        if cached is not None:
            cached['ref_count'] -= 1
            if cached['ref_count'] <= 0:
                del self._cache[index]
            clean_audio, noisy_audio = cached['clean'], cached['noisy']
        else:
            ...
            if self.n_cache_reuse > 0:
                self._cache[index] = {
                    'clean': clean_audio,
                    'noisy': noisy_audio,
                    'ref_count': self.n_cache_reuse,
                }
        ...
        assert clean_audio.size(0) == noisy_audio.size(0)

        if self.split:
            if clean_audio.size(0) >= self.segment_size:
                max_audio_start = clean_audio.size(0) - self.segment_size
                ...
                clean_audio = torch.nn.functional.pad(clean_audio, (0, self.segment_size - clean_audio.size(0)), 'constant')
                noisy_audio = torch.nn.functional.pad(noisy_audio, (0, self.segment_size - noisy_audio.size(0)), 'constant')
